...
// 🔹 Optimized Auto-Expanding Title Input to prevent lag/cursor jump and allow multi-line wrap
const TitleInput = ({ initialValue, onUpdate }) => {
    ...
    // Recalculate height on window resize or modal animation
    useEffect(() => {
        ...
        if (selectedTicket && selectedTicket._id !== 'new') {
            ...
            if (selectedTicket.recurringTaskId) {
                getRecurringTaskById(selectedTicket.recurringTaskId)
                    .then(res => {
                        if (res && res.data) {
                            // Use the actual enabled value from the DB — do NOT hardcode true
                            const isEnabled = typeof res.data.enabled === 'boolean' ? res.data.enabled : true;
                            setRecurringConfig({ ...res.data, enabled: isEnabled });
                        }
                    })
                                    ...
                                    >
                                        ...
                                        <div className="flex justify-between items-center mb-1">
                                            <div className="flex flex-wrap gap-1.5 items-center">
                                                ...
                                                <span className={`px-1.5 py-0.5 text-[9px] font-bold uppercase rounded border tracking-wider shrink-0 ${status === 'Done' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' :
                                                        ...
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5 w-full">
                                                            <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 h-8 focus-within:ring-1 focus-within:ring-teal-500 focus-within:bg-white shadow-2xs">
                                                                ...
                                                                <input
                                                                    ...
                                                                    className="w-full bg-transparent text-xs font-bold text-slate-700 outline-none cursor-pointer p-0 border-none focus:ring-0"
                                                                    title="Start Date"
                                                                />
                                                            </div>
